...
class CSARestClient
  ...
  def display_users
    begin
      response = RestClient.get "#{@@DOMAIN}/api/users.json"
  ...
  def update_user
    begin
      ...
      response = RestClient.put "#{@@DOMAIN}/api/users/#{id}.json", { user: result }
  ...
  def delete_user
    begin
      print "Enter the user ID: "
      id = STDIN.gets.chomp
      response = RestClient.delete "#{@@DOMAIN}/api/users/#{id}.json"
