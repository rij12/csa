# Represents a user account detail that maintains user credentials
# and performance authentication checks
# @author Chris Loftus
require 'digest/sha2'
class UserDetail < ApplicationRecord
  ...
  def self.authenticate(name, passwd)
    ...
    if user_detail
      expected_password =
          UserDetail.encrypt_password(passwd, user_detail.salt)
